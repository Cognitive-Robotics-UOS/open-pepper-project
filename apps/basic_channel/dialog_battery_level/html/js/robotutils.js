...
RobotUtils = (function(self) {
    ...
    self.connect = function(connectedCallback, failureCallback) {
        ...
        getScript(robotlibs + 'qimessaging/2/qimessaging.js', function() {
            ...
            var error;
            if (self.robotIp) {
                error = "Failed to get qimessaging.js from robot: " + self.robotIp;
            } else {
                error = "Failed to get qimessaging.js from this domain; host this app on a robot or add a ?robot=MY-ROBOT-IP to the URL.";
            }
            console.error(error);
            if (failureCallback) {
