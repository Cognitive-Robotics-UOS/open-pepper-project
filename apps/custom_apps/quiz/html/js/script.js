...
var SP_INFO = 'Thanks for being interested in me and the humans who work on \
               me! I\'ve been with the Cognitive Science students in Osnabrueck \
               since August 2017. Last year, my humans trained me as part of \
               the Cognitive Robotics study project to become a neuroscience \
               tutor. This semester, their goals have broadened even more: \
               They\'re working very hard to make me have small talk with you \
               humans in a natural way. Hopefully, I can then act as a \
               university guide to new students soon. This will include giving \
               them directions, teaching them about the university and \
               Cognitive Science and of course entertaining them with games \
               and magic tricks!';
...
function startGame() {
  ...
  if (!game.german) $('#header').attr('src', MEDIA_URL + 'cogsci-ibm.png');
  else $('#header').attr('src', MEDIA_URL + 'ghcampus-header.png');
  setLanguage();
  setLangParams();
  // Add click event listeners
  addClickListeners();
  $('#game').show('slow').promise().done(function() {
    if (!game.german) {
...
/** Replay current video in case the current story is not spoken til the end */
function videoEnded() {
  // services.tablet.then(function(tablet) {
  //   tablet.stopVideo();
  // });
...
/** Subscribe the NAOqi signal ALTabletService.videoFinished into videoEnded
 * function.
 */
function subscribeVideoFinished() {
  services.tablet.then(function(tablet) {
    tablet.videoFinished.connect(videoEnded);
  });
}
 ...
 * @return {Object} - Contains the read raw data from all TSV files
 */
...
$(document).ready(function() {
  // Materializecss initialization for collapsible class elements
